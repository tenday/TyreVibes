-- ============================================
-- SETUP DATABASE PRONTO ALL'USO
-- TyreVibes Background Jobs System
-- ============================================
--
-- ISTRUZIONI:
...
-- Rimuovi job esistenti se presenti (per evitare duplicati)
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'run-all-background-jobs') THEN
        PERFORM cron.unschedule('run-all-background-jobs');
    END IF;
END $$;

-- Job principale che esegue tutti gli aggiornamenti alle 07:00 ogni giorno
-- ⚠️ NOTA: Le Edge Functions devono essere deployate prima che questo funzioni!
-- Per ora lo creiamo, verrà attivato dopo il deploy delle functions
...
LEFT JOIN vehicle_insurances vi ON vi.plate_id = v.id
WHERE
...
GRANT SELECT ON bollo_status TO authenticated;
GRANT SELECT ON revision_status TO authenticated;
GRANT SELECT ON upcoming_expirations TO authenticated;
...
-- Query di verifica
SELECT
    '✅ Setup database completato con successo!' as message,
    (SELECT COUNT(*) FROM information_schema.tables WHERE table_name IN ('bollo_status', 'revision_status', 'job_execution_log')) as tables_created,
    (SELECT COUNT(*) FROM cron.job WHERE jobname = 'run-all-background-jobs') as cron_jobs_active;

-- Mostra il cron job creato
SELECT
    jobname as job_name,
    schedule,
    active,
...
-- ============================================
-- PROSSIMI PASSI:
-- 1. ✅ Database setup completato
-- 2. ⏳ Deploy delle Edge Functions (vedi DEPLOYMENT_GUIDE.md)
-- 3. ⏳ Test manuale delle funzioni
